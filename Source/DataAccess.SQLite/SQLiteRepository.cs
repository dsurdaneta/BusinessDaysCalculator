...
public class SQLiteRepository : IRepository<DbModels.Holiday>
{
    ...
    public SQLiteRepository(HolidaysSQLiteContext context) => this.dbContext = context;
    ...
    /// <inheritdoc />
    public async Task<ICollection<DbModels.Holiday>> GetAllAsync() => await this.dbContext.Holidays.ToListAsync().ConfigureAwait(false);

    /// <inheritdoc />
    public async Task<DbModels.Holiday> GetByIdAsync(int id) => await this.dbContext.Holidays.FirstOrDefaultAsync(x => x.Id.Equals(id))
            .ConfigureAwait(false);

    /// <inheritdoc />
    public ICollection<DbModels.Holiday> Get(Func<DbModels.Holiday, bool> expression) => this.dbContext.Holidays.AsParallel().Where(expression).ToList();
    ...
    /// <inheritdoc />
    public async Task<bool> AnyAsync(DbModels.Holiday entity) => await this.dbContext.Holidays.AnyAsync(x => x.Id.Equals(entity.Id) || x.HolidayDate.Equals(entity.HolidayDate))
            .ConfigureAwait(false);
}
